...
enum ModelSelectionStore {
    ...
    /// Current user selection. Defaults to `.gpt55` — the historical default —
    /// so behavior is unchanged for users who never open the Model menu.
    static var current: ModelSelection {
        get {
            let raw = iCloudKVSDefaults.shared.string(forKey: defaultsKey) ?? ""
            return ModelSelection(rawValue: raw) ?? .gpt55
        }
        set {
