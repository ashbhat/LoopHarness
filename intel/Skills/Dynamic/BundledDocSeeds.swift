...
- **Loop** — the iOS app (UIKit). Compiles the shared `intel/` folder. Chat
  UI lives in `MessagingVC`.
- **LoopMac** — the macOS app. Compiles `intelmac/` plus most of `intel/`.
  The voice loop (record → transcribe → think → speak) is driven by
  `intelmac/VoiceLoopCoordinator.swift`.
- **LoopShare** — a share extension for sending content into the workspace.

`intel/` is a *synchronized folder group*: any new source file added there is
...
`intel/AgentHarness/AgentHarness.swift` is the brain. It owns the markdown
...
1. The global `tools` array in `intel/Structs/Messaging.swift` (schemas).
2. The prompt-fragment composition + skill catalog in
   `intel/AgentHarness/AgentHarness.swift`.
3. `intel/AgentHarness/SkillDispatcher.swift` — the headless dispatcher used
   by background/scheduled runs and sub-agents.
4. `intel/MessagingVC.swift` — the iOS chat `processMessage` chain.
5. `intelmac/VoiceLoopCoordinator.swift` — the Mac voice `dispatchFunction`.

## The Workspace (the agent's file system)

`intel/Workspace/Workspace.swift` resolves a single root directory — preferring
...
`intel/SubAgents/` implements detached background workers. The primary agent
...
`intel/Data/Cloud.swift` is the network layer. `Cloud.connection.chat(...)`
