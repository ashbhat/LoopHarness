...
//
//  Most skills are visible from the shared `intel/` target so the dispatcher
...
//  name without `intel/` ever importing AppKit.
//
...
final class SkillDispatcher {
    ...
    /// can't be referenced from `intel/` directly). Bundled iOS skills are
